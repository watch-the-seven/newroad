...
6. **卖矿阈值改为"铜+铁合计 20"，且触发后把铜铁全部清空**。口述脚本是"某一种矿攒
   够 20 就卖"，那会让背包里最多同时压着铜 20 + 铁 20 = 40 格；改成合计阈值后上限
   只有 20 格（实测背包峰值 58 → 40）。因为接口的 `sell` 只接受单个 `name`
   （接口文档 2.2/2.3），所以"两种矿都有"时必然要用两个回合卖完，实现上用
   `MatchState.sell_pending` 记住"正在清仓"——否则卖掉第一种后合计掉到 20 以下，
   第二种就漏卖了。石头不参与这条规则。
7. `x == 20` 这种边界情况归入右侧脚本。
